...
public class Calculate1 implements Calculate {
	...
	public Calculate1(Integer[][][] segments) {
	...
	private void addIntersections(List<Point> points) {
		for (Point point : points) {
			if (!this.intersections.containsKey(point)) {
				this.intersections.put(point, 0);
			}
			Integer value = this.intersections.get(point);
			this.intersections.put(point, value+1);
		}
	}
